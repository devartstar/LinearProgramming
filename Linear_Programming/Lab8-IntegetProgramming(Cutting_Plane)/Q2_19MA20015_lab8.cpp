/*
    ...
    Roll No: 19MA20015
    Lab No: 8
    Q2 cutting plane method with integer solution for only single variable
*/
// program to perform cutting plane method to find integer value for a specific variable in the optimal solution
#include <iostream>
#include <vector>
#include <cmath>
#include <array>
...
int take_input(VVD &coeff_mat, VS &basic, VS &nbasic)
{
    ...
    // taking a variable for which integer variable is required
    cout << "Enter the variable for which integrall value is needed(eg: x2): ";
    cin >> int_var;
    if (int_var[0] != 'x')
    {
        cout << "INVALID input!\nTaking default value as x2";
        int_var = "x2";
    }
...
int cutting_plane_method(VVD coeff_mat, VS basic, VS nbasic, int &curr_iter, bool optimal)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        if (fract > max && fract != 0 && basic[i] == int_var)
        {
            max = fract;
            row = i;
        }
