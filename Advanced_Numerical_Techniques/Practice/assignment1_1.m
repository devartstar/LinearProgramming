%{
    ...
    F1, F2, .... FN-1 -> using i=1,2,..,n in equation <3>
    ---------------------------------
...
function BVP1()
    ...
    disp(rhsMatrix);
    sol= guassElimination(coefMatrix, rhsMatrix, n);
...
%% Guass Elimination
function y = guassElimination(A, B, n)
    Ab = [A, B];
    y = zeros(n+1, 1);
    y(n+1) = Ab(n+1,end) / Ab(n+1,n+1);
    for i = n : -1 : 1
        y(i) = (Ab(i,end) - Ab(i,i+1:n) * y(i+1:n)) / Ab(i,i);
    end
end
