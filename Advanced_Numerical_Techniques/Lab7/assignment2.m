...
function assignment2()
    ...
    %{
        Using the given data points we find value of M1, M2, M3
        Considering Natural spline M0 = M4 = 0
        Using the formulae
        M(i-1) + 4 * M(i) + M(i+1) = 6/h^2 (y(i-1) - 2y(i) + y(i+1))
        i=1 Eq1 -> 4M1 + M2 = -12
        i=2 Eq2 -> M1 + 4M2 + M3 = 12
        i=3 Eq3 -> M2 + 4M3 = -12
    %}
    ...
    % To print the values asked y(1.5) and y(3.5)
    fprintf("Solution :- \n");
    fprintf("y (%f) = %f \n",1.5,calcValue(1.5,X,Y,M,h));
    fprintf("y (%f) = %f \n",3.5,calcValue(3.5,X,Y,M,h));
end
